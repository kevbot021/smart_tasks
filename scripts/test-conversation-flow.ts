...
// Move function outside the main function to fix strict mode error
async function getAssistantResponse(
  ...
  const run = await openai.beta.threads.runs.create(threadId, {
    ...
    instructions: "Analyze the task and provide guidance."
  });
...
async function testConversationFlow() {
  try {
    ...
    const initialResponse = await getAssistantResponse(openai, assistantId, thread.id);
    console.log('\n📥 Initial response received');

    // 4. Simulate user selection
    console.log('\n📤 Selecting option: "Wooden"...');
    const followUp1 = await getAssistantResponse(openai, assistantId, thread.id, "Wooden");
    console.log('\n📥 Follow-up response received');

    // 5. Another user selection
    console.log('\n📤 Selecting option: "Yes, I have some DIY experience"...');
    const followUp2 = await getAssistantResponse(openai, assistantId, thread.id, "Yes, I have some DIY experience");
